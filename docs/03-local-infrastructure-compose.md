...
Raw Compose works, but it must be run from `deploy/compose/` (or with `-f`), and "container started" is not the same as "ready". So use the helper script below; it checks prerequisites, warns about ports already in use, and waits for every health check.
...
scripts/infra.sh status      # per-service health
scripts/infra.sh logs keycloak
scripts/infra.sh tools       # optional Kafka UI on http://localhost:8090
scripts/infra.sh restart keycloak   # re-create it (re-imports the realm after you edit the JSON)
scripts/infra.sh down        # stop, keep data
scripts/infra.sh reset       # stop and WIPE data (asks first)
```
...
> **Port clashes are the most common startup failure.** `infra.sh up` warns when 5432, 27017, 6379, 9092, 8180 or 8000 is taken, and names the container holding it, which is often an unrelated local cluster or a service you left running in your IDE.
